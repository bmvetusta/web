...
export const responseOfficialReportSchema = z
  .object({
    acta: z.object({
      ...
      jugadores: z.array(playerOfficialReportSchema),
      goles: z.array(goalOfficialReportSchema).default([]),
      admonestaciones: z.array(faultOfficialReportSchema).default([]),
      eventos: z.array(eventOfficialReportSchema).default([]),
      sustituciones: z.array(z.any()).default([]),
      minutos_jugados: z.coerce.number().default(0),
      numero_partes: z.coerce.number().default(0),
      urlActaPartido: z.string().url().nullable().default(null),
      estadisticasPartido: z.array(matchStatsOfficialReportSchema).default([]),
